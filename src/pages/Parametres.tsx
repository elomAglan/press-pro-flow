...
export default function Parametres() {
  ...
  // ✅ Charger le pressing au montage
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const list = await getAllPressings();
        if (list.length > 0) {
          const p = list[0];
          setPressing({ ...p, logoBase64: p.logo });
        }
      } catch (e) {
        console.error(e);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPressing({ ...saved, logoBase64: form.logoBase64 });
        ...
        <div className="p-4 border rounded-lg shadow bg-white">
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-full bg-gray-200 overflow-hidden">
              {pressing.logoBase64 ? (
                <img src={pressing.logoBase64} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="mt-4 space-y-2">
            <p className="flex items-center gap-2">
              <Mail size={16} /> {pressing.email}
            </p>
            <p className="flex items-center gap-2">
              <Phone size={16} /> {pressing.telephone}
            </p>
            <p className="flex items-center gap-2">
              <MapPin size={16} /> {pressing.adresse}
            </p>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-full max-w-lg shadow">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {form.logoBase64 && (
                <img src={form.logoBase64} className="h-20 mt-2 rounded" />
              )}
