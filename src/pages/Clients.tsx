...
type ClientStatus = 'Actif' | 'Inactif'; // 'VIP' a été consolidé en 'Actif'
...
const initialClients: Client[] = [
    { id: "CL001", nom: "Dupont Jean", telephone: "0612345678", email: "jean.dupont@mail.com", adresse: "15 Rue de la Paix, Paris", createdAt: new Date(2023, 0, 15), status: 'Actif' }, // Était 'VIP'
    ...
    { id: "CL004", nom: "Dubois Alice", telephone: "0711223344", email: "alice.dubois@mail.com", adresse: "10 Rue du Commerce, Lille", createdAt: new Date(2024, 1, 1), status: 'Actif' }, // Était 'VIP'
    ...
    { id: "CL009", nom: "Moreau Chloé", telephone: "0799887766", email: "chloe.moreau@mail.com", adresse: "1 Impasse Bleue, Bordeaux", createdAt: new Date(2024, 5, 1), status: 'Actif' }, // Était 'VIP'
...
type FilterType = 'all' | 'actif' | 'inactive'; // Ajout de 'actif' et suppression de 'vip'
...
export default function Clients() {
    ...
    const [filterBy, setFilterBy] = useState<FilterType>('all'); // Nouveau state pour le filtre

    // Mappe le filtre interne vers le statut du client
    const statusMap: Record<FilterType, ClientStatus | null> = {
        'all': null,
        'actif': 'Actif', // Nouveau filtre actif
        'inactive': 'Inactif'
    };
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-8 space-y-8 font-sans">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">Clients</h1>
                ...
                <div className="flex gap-3">
                    ...
                    <Button 
                        ...
                        className="gap-2 bg-yellow-500 hover:bg-yellow-600 text-white shadow-md hover:shadow-lg transition-all"
                    >
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={onOpenChange}>
                        ...
                        <DialogContent className="sm:max-w-[500px] bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6">
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-bold text-gray-800 dark:text-white">{dialogTitle}</DialogTitle>
                            </DialogHeader>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    >
                        <Users className="h-4 w-4 mr-2 inline" /> Tous les clients
                    </button>
                    <button 
                        onClick={() => setFilterBy('actif')} 
                        className={getFilterButtonClass('actif')}
                    >
                        <Star className="h-4 w-4 mr-2 inline text-green-400" /> Clients Actifs
                    </button>
                    <button 
                        onClick={() => setFilterBy('inactive')} 
                        className={getFilterButtonClass('inactive')}
                    >
                        <UserX className="h-4 w-4 mr-2 inline" /> Clients Inactifs
                    </button>
                </div>
            </div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
                <Card className="p-6 shadow-xl border border-gray-100 dark:border-gray-700">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Clients Actifs</p>
                    <p className="text-3xl font-extrabold text-green-600 dark:text-green-400 mt-1">
                        {clients.filter(c => c.status === 'Actif').length}
                    </p>
                </Card>
                <Card className="p-6 shadow-xl border border-gray-100 dark:border-gray-700">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Clients Inactifs</p>
                    <p className="text-3xl font-extrabold text-red-600 dark:text-red-400 mt-1">
                        {clients.filter(c => c.status === 'Inactif').length}
                    </p>
                </Card>
                <Card className="p-6 shadow-xl border border-gray-100 dark:border-gray-700">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Résultats affichés</p>
                    <p className="text-3xl font-extrabold text-blue-600 dark:text-blue-400 mt-1">{filteredClients.length}</p>
                </Card>
            </div>

            {/* Clients Table (Tableau) */}
            <Card className="p-0 overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-700">
                {/* Conteneur pour le défilement vertical */}
                <div className="w-full overflow-x-auto max-h-[600px] overflow-y-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 shadow-sm z-10">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[10%]">ID</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[20%]">Nom</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[15%] flex items-center gap-1"><Phone className="h-3 w-3" /> Téléphone</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[15%]">Statut</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[25%] flex items-center gap-1"><Mail className="h-3 w-3" /> Email</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[15%]">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {filteredClients.length > 0 ? (
                                filteredClients.map((client) => (
                                    <tr key={client.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition duration-150">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-white">{client.nom}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{client.telephone}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm">{getStatusBadge(client.status)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 dark:text-blue-400 truncate max-w-xs">{client.email}</td>
